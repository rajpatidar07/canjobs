...
export default function EmployeeSignupModal(props) {
  ...
  const validators = {
    ...
    reffer_by: [
      (value) =>
        value === "" || value === null || typeof state.reffer_by === "object"
      ...
      if (isChecked) {
        ...
        try {
          ...
          if (signUpData.message === "Employee has been registered") {
            ...
            try {
              let Response = await AddEmployeePermission(updatedPermissions);
  ...
  useEffect(() => {
    let i = i + 3; // Assuming `i` is defined somewhere else
    if (
      (code !== "" ||
        code !== undefined ||
        code !== "undefined" ||
    ...
    AgentJson(); // Assuming AgentJson is defined somewhere else
  }, []);
