...
function SinUpPage() {
    ...
    contact_no: [
      (value) =>
        value === ""
          ? ""
          : value.length > 20
          ? "Contact number should be less then 20 digit"
          : "",
    ],
    // resume: [
  ...
  axios
    .post(
      `https://www.linkedin.com/oauth/v2/accessToken?code=${code}&grant_type=authorization_code&client_id=78mhwjaumkvtbm&client_secret=ZoZKbJgORl0vYJFr&redirect_uri=${window.location.origin}`
    )
    .then((response) => {
