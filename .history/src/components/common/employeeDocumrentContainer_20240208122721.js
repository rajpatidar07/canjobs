...
export default function DocumrentContainer(props) {
  ...
  const addAnnotation = async (annotation) => {
    ...
    let assignedAdminName = allAdmin.find(
      ...
      : "";
      // const formattedEmails = allAdmin.filter(item => selectedAdmin.includes(item.email)).map(admin => admin.email).join(', ');

    console.log(
      allAdmin.filter(item => selectedAdmin.includes(item.email)).map(admin => admin.email).join(', ');
    );
