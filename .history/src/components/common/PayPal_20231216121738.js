...
//   console.log("b4-----------------" + payAmt);
//   const createOrder = (data, actions) => {
//     console.log(data);
//     console.log("after-----------------" + payAmt);
//     return actions.order.create({
//       purchase_units: [
//         {
//           amount: {
//             value: payAmt, // Example amount (update with your actual amount)
//           },
//         },
//       ],
//     });
...
const PayPalButton = (props) => {
  // const [payAmt, setPayAmt] = useState(props.amount);
  console.log("run-----onchangeInput-----" + props.amount);
  const createOrder = async (data, actions) => {
    let amt = await props.getAmt();
    console.log("chk---amt" + amt);
    return actions.order.create({
      purchase_units: [
        {
          amount: {
            value: amt, // Example amount (update with your actual amount)
          },
        },
      ],
    });
  ...
  const onApprove = (data, actions) => {
    ...
    return actions.order.capture().then(async function (details) {
      ...
      if (details.status === "COMPLETED") {
        ...
        try {
          let res = await AddPayPalpay(data);
          console.log(res);
        } catch (err) {
          console.log(err);
        }
