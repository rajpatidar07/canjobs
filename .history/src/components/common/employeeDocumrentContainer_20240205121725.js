...
export default function DocumrentContainer(props) {
  ...
  const handleInputChange = (event, type) => {
    ...
    // Check if the input value contains "@" after a space
    if (value.includes(" @")) {
      // Handle admin mention
      AdminData(); // Assuming this function fetches admin data
      if (allAdmin) {
        // Filter admin emails based on input
        const filteredAdminEmails = allAdmin.filter((admin) =>
          admin.email
            .toLowerCase()
            .includes(value.trim().slice(1).toLowerCase())
        );
        console.log(filteredAdminEmails);
        // Update the filtered emails
        setFilteredEmails(filteredAdminEmails);
      }
    } else {
      // Reset filtered emails if "@" is not after a space
      setFilteredEmails([]);
    }

    // Update the input value based on the type (reply or regular comment)
    if (type === "reply") {
      // Set reply comment
      setReplyComment(value);
    } else {
