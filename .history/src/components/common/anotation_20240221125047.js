...
const Annotation = () => {
  ...
  const handleAnnotationSubmit = () => {
    ...
    const newAnnotation = {
      text,
      // Additional data such as user ID, timestamp, etc. can be added here
    };
  ...
  const renderAnnotations = () => {
    return annotations.map((annotation, index) => (
      <div key={index} className="annotation">
        ...
        {/* Additional annotation details such as user info, timestamp, etc. can be displayed here */}
      </div>
    ));
  };
  ...
  return (
    <div>
      ...
      <div className="annotation-list">{renderAnnotations()}</div>
    </div>
  );
};
