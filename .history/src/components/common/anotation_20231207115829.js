...
const Annotation = () => {
  ...
  const addAnnotation = ({ x, y }) => {
    ...
    const id = localStorage.getItem("adminId"); // Replace with actual method to get admin ID
    const docId = props.docId; // Assuming docId is passed as a prop
    const assignedUserId = assignments.find(
      (assignment) => assignment.docId === docId
    )?.assignedUserId;
    const email = isEmailConditionMet() ? props.email : null;
    const subject = props.subject; // Replace with actual value
    const comment = isEmailConditionMet() ? null : props.comment; // Replace with actual value
