  ...
  const GetDocument = async () => {
    try {
      ...
      if (
        ...
        setLoading(false);
        if (
          response.data.data.map((item) => item.type === docName) ||
          response.data.data.map((item) => item.id === docId)
        ) {
          setDocId(response.data.data.map((item) => item.id === docId).id);
          setDocFile(
            response.data.data.map((item) => item.id === docId).document_url +
