...
export default function EmployerDocumrentContainer(props) {
  ...
  const handleCommentClick = (annotation) => {
    const flag = imageAnnotations.find(
      (a) => a.x_axis === annotation.x_axis && a.y_axis === annotation.y_axis
    );

    if (flag) {
      setSelectedAnnotation(flag);
    }
    }
  };
  return (
          ...
          >
            ...
            <ul>
              {(commentsList || []).map((commentItem, index) => (
                <Link to="" onClick={() => handleCommentClick(commentItem)}>
                  {" "}
