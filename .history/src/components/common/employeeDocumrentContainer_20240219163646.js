...
export default function DocumrentContainer(props) {
  ...
  const handleInputChange = (event) => {
    ...
    setComments(inputValue); // Update the input value state

    // Check if the last character is '@'
    if (inputValue.endsWith("@")) {
      AdminData(); // Fetch admin data
      if (allAdmin) {
        // Filter admin emails based on input value
        const filteredAdminEmails = allAdmin.filter((admin) =>
          admin.email.toLowerCase().includes(inputValue.toLowerCase())
        );
        setFilteredEmails(filteredAdminEmails); // Update the filtered emails state
      }
    } else {
      setFilteredEmails([]); // Clear filtered emails state if input doesn't end with '@'
    }
  };

  // Handle click event on email suggestion
  const handleEmailClick = (email) => {
    setComments((prevComments) => prevComments + email); // Append the selected email to the input value
    setFilteredEmails([]); // Clear filtered emails state
  };
