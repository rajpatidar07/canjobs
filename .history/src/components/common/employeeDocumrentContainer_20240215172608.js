...
export default function DocumrentContainer(props) {
  ...
  const GetDocument = async (del) => {
    try {
      ...
      const allData = response.data.data.allData || [];
      ...
      if (del || !docTypData || !docName || otherDoc === false) {
        // eslint-disable-next-line
        const selectedDoc =
          allData.find((item) => item.id === docId) || allData[0];

        docTypData = selectedDoc;
        docFile = selectedDoc
          ? (selectedDoc.document_url || "") +
            `?v=${new Date().getMinutes() + new Date().getSeconds()}`
          : "";
        docName = selectedDoc ? selectedDoc.document_name || "" : "";
        docId = selectedDoc ? selectedDoc.id || "" : "";
        docTypeName = selectedDoc ? selectedDoc.type || "" : "";
      } else if (
        !showMoreDocType &&
        allData.find((item) => item.document_name === docName)
      ) {
        const selectedDoc = allData.find(
          (item) => item.document_name === docName
        );
        ...
        //Condition for update
        const selectedDoc = allData.find((item) => item.id === docId);
