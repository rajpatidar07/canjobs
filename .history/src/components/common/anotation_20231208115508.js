...
import "./Annotation.css"; // Import a CSS file for styling
...
const Annotation = () => {
  ...
  // State to keep track of the last "@" mention in the input
  const [mentionIndex, setMentionIndex] = useState(-1);

  // State to store the selected emails
  const [selectedEmails, setSelectedEmails] = useState([]);

  // State to store the filtered admin emails
  ...
  const handleInputChange = (event) => {
    ...
    const newMentionIndex = value.lastIndexOf("@");
    setMentionIndex(newMentionIndex);

    if (newMentionIndex !== -1) {
      // Extract the text following the last "@" mention
      const mentionText = value.slice(newMentionIndex + 1);

      // Filter admin emails based on mention text
      const filteredAdminEmails = adminEmails.filter((email) =>
        email.toLowerCase().includes(mentionText.toLowerCase())
      );

      // Update the filtered emails
      setFilteredEmails(filteredAdminEmails);
    } else {
      // Reset filtered emails if there is no "@" mention
      setFilteredEmails([]);
    }
  ...
  const handleEmailClick = (email) => {
    // Remove "@" before setting the selected email as the input value
    setInputValue((prevValue) => {
      const newValue =
        mentionIndex !== -1
          ? prevValue.substring(0, mentionIndex) + email
          : email;
      setMentionIndex(-1); // Reset mention index after selecting an email
      return newValue;
    });
  ...
  const handleRemoveEmail = (email) => {
    // Remove the email from the selected list
    ...
    <div className="annotation-container">
      ...
      <div className="input-container">
        {selectedEmails.map((selectedEmail) => (
          <div key={selectedEmail} className="selected-email">
            {selectedEmail}
            <span
