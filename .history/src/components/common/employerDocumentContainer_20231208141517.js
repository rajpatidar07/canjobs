...
export default function EmployerDocumrentContainer(props) {
  ...
  const handleInputChange = (event) => {
    ...
    if (value.startsWith("@")) {
      ...
      const filteredAdminEmails = allAdmin.filter((email) => {
        console.log(email);
        // Check if email and email.email are defined
        ...
        return false; // Handle the case where email or email.email is undefined
      });
  ...
  }, [docName, apiCall, isAnnotationMode, docId]);
  ...
  const addAnnotation = async (annotation) => {
    ...
    console.log(allAdmin.filter((item) => item.email === comments));
    const assignedUserId = allAdmin.filter((item) => item.email === comments);
                ...
                >
                  <li key={index} className="text-break">
                    ...
                    {commentItem.assigned_to && (
                      <div
                        style={{
                          backgroundColor: " rgb(209 239 214)",
                          color: " white",
                          borderRadius: " 15px",
                          padding: " 5px 10px",
                          margin: " 5px",
