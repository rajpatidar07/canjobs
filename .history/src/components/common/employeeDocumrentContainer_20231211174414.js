...
export default function DocumrentContainer(props) {
  ...
  const determineBackgroundColor = (commentItem) => {
    ...
    const hash = Math.abs(hashCode(assignedUserId.toString()));
    const index = hash % (colorClasses.length + 1);

    // Check if assigned_to_user_id is present and is the same as the commentItem's assigned_to_user_id
    if (assignedUserId && assignedUserId === commentItem.assigned_to_user_id) {
      return colorClasses[0]; // Use the first color for matching assigned_to_user_id
    }
