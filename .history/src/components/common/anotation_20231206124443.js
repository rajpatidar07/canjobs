...
const Annotation = () => {
  ...
  const handleMouseDown = () => {
    if (isAnnotationMode) {
      setCurrentAnnotation({ x: 0, y: 0, text: "" });
    }
  };

  const handleMouseUp = () => {
    if (isAnnotationMode && currentAnnotation.text.trim() !== "") {
      setAnnotations([...annotations, currentAnnotation]);
      setCurrentAnnotation({ x: 0, y: 0, text: "" });
    }
  };
  ...
  const handleAnnotationClick = (annotation) => {
    setSelectedAnnotation(
      selectedAnnotation === annotation ? null : annotation
    );
  };
  ...
  return (
    <div style={{ position: "relative", overflow: "scroll" }}>
      <canvas
        ...
        onMouseDown={handleMouseDown}
        onMouseUp={handleMouseUp}
        onMouseUp={handleAnnotation}
        style={{
          border: "1px solid #000",
          userSelect: isAnnotationMode ? "text" : "none",
        }}
      ></canvas>
