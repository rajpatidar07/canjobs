...
const GoogleDrive = () => {
  ...
  const handleOpenDocument = (document) => {
    ...
    // Implement logic to open the selected document (e.g., redirect to a viewer)
...
const GoogleDrive = () => {
  ...
  const handleFileChange = (event) => {
    const file = event.target.files[0];
    setSelectedDocumentContent(null); // Reset selected document content
  ...
  const handleOpenDocument = (fileId) => {
    gapi.client.drive.files
      .get({
        fileId: fileId,
        alt: "media", // Request the media content of the file
      })
      .then((response) => {
        const documentContent = response.body;
        setSelectedDocumentContent(documentContent);
      })
      .catch((error) => {
                ...
                <FileListItem
                  ...
                  onClick={() => {
                    if (document.mimeType === "application/vnd.google-apps.folder") {
                      listFiles(document.id); // If the clicked document is a folder, list its contents
                    } else {
                      handleOpenDocument(document.id); // Handle opening/viewing non-folder documents
                    }
                  }}
                >
                  {document.name}
          ...
          {selectedDocumentContent && (
            <div>
              ...
              <iframe
                ...
                srcDoc={selectedDocumentContent}
                width="100%"
                height="500px"
              />
            </div>
