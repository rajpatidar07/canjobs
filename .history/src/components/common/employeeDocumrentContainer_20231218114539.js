...
export default function DocumrentContainer(props) {
  ...
  const handleInputChange = (event, type) => {
    ...
    if (value.includes("@")) {
      // Extract email after '@' following whitespace
      const atIndex = value.indexOf("@");
      const lastSpaceIndex = value.lastIndexOf(" ", atIndex);
      const start = lastSpaceIndex !== -1 ? lastSpaceIndex + 1 : 0;
      const email = value.slice(start, atIndex);

      // Update the email state
      setEmail(email);
    } else {
      // Reset email state if input doesn't include '@'
      setEmail("");
    }
