...
export default function DocumrentContainer(props) {
  ...
  //   setFilteredEmails([]);
  // };
  const delimiter = "@$"; // Choose a delimiter that won't conflict with user input

  const handleInputChange = (event, type) => {
    const value = event.target.value.trim();
    const words = value.split(" ");
    const assignAdminWords = words.filter((word) => word.startsWith("@"));

    if (assignAdminWords.length > 0) {
      // Extract admin emails
      const adminEmails = assignAdminWords.map((word) => word.slice(1));
      ...
      // Reset filtered emails if no "@" after a space
      setFilteredEmails([]);
    }
  ...
  /* Function to handle assigning admin on click */
  const handleEmailClick = (email, type) => {
    // Set the selected admin and update the input value
    const commentValue = type === "reply" ? replyComment : comments;
    const updatedValue =
      commentValue.replace(`@${email}`, "").trim() + ` ${delimiter}@${email}`;
