...
export default function DocumrentContainer(props) {
  ...
  let SaveBulkDocument = async () => {
    try {
      let response = await UploadBulkDocument(props.employee_id, docFileBase);
      ...
      if (response.data.message === "all fields required !") {
      ...
      <div className="row m-0">
        ...
        >
          <div className="row px-0 pt-0 pb-5 doc_upload_row m-0">
            ...
            {showSaveDoc ? (
              <div className="doc_upload_col">
                {docTypData.id<button className="btn btn-primary" onClick={SaveDocument}>
                  Update Document
                </button>: <button className="btn btn-primary" onClick={SaveBulkDocument}>
                  Save Documents
                </button>}
