  ...
  const GetDocument = async () => {
    try {
      ...
      if (
        ...
        setLoading(false);
        if (
          response.data.data.filter((item) => item.type === docName) ||
          response.data.data.filter((item) => item.id === docId)
        ) {
          setDocTypData(
          ...
          setDocId(response.data.data.find((item) => item.type === docName).id);
          setDocFile(
            response.data.data.find((item) => item.type === docName)
              .document_url +
