...
export default function DocumrentContainer(props) {
  ...
  const handleInputChange = (event, type) => {
    ...
    if (value.startsWith("@")) {
      // Check if '@' is present in the comment
      ...
      if (atIndex !== -1) {
        ...
        const filteredAdminEmails = allAdmin.filter((admin) => {
          ...
          if (admin && admin.email) {
            ...
            return admin.email
              .toLowerCase()
              .includes(value.slice(atIndex + 1).toLowerCase());
          }
        ...
        // Set the start index of the mention
        setMentionStartIndex(atIndex);

        // Trim the comment and check if it starts with "@" and matches the email format
        if (type === "reply") {
          setReplyComment(value.slice(atIndex + 1));
        } else if (type !== "reply") {
          setComments({
            ...comments,
            [`${mentionStartIndex}-${atIndex}`]: value.slice(atIndex + 1),
          });
        }
      } else {
        // Reset filtered emails if input doesn't have '@'
        setFilteredEmails([]);
        // Reset the mention start index
        ...
        if (type === "reply") {
          ...
          setComments({
            ...comments,
            [`${mentionStartIndex}-${atIndex}`]: value,
          });
        }
      }
    }
