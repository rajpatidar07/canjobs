...
export default function DocumrentContainer(props) {
  ...
  //       let filteredAdminEmails;
  //       value.includes(" @")
  //         ? allAdmin.filter((admin) => admin.email.toLowerCase())
  //         : (filteredAdminEmails = allAdmin.filter((admin) =>
  ...
  const handleInputChange = (event, type) => {
    ...
    if (atIndex !== -1) {
      ...
      // Check if there's a space after '@' symbol
      const spaceIndex = afterAt.indexOf(" ");
      const selectedEmailOrComment =
        spaceIndex !== -1 ? afterAt.substring(0, spaceIndex) : afterAt;

      // Filter admin emails based on input
      const filteredAdminEmails = allAdmin.filter((admin) =>
        admin.email.toLowerCase().includes(selectedEmailOrComment.toLowerCase())
      );

      // Update the filtered emails
      setFilteredEmails(filteredAdminEmails);

      // Concatenate both the previous value of email and the newly selected email/comment
      const updatedValue = beforeAt + "@" + selectedEmailOrComment;

      // Update the input value
      if (type === "reply") {
        setReplyComment(updatedValue);
      } else {
        // Preserve the comment if it exists, otherwise just set the email
        setComments(comments ? comments + " " + updatedValue : updatedValue);
      }
    } else {
      // Reset filtered emails if input doesn't contain '@'
      setFilteredEmails([]);
