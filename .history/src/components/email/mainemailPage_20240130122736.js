  ...
  const EmailData = async () => {
    try {
      ...
      if (
        ...
        const skiptokens = userData.data.paginationData.map((url) => {
          const skiptokenStartIndex =
            url.indexOf("$skiptoken=") + "$skiptoken=".length;
          console.log(skiptokenStartIndex);
          // const skiptokenEndIndex = url.indexOf("&", skiptokenStartIndex);
          // return url.substring(
          //   skiptokenStartIndex,
          //   skiptokenEndIndex !== -1 ? skiptokenEndIndex : undefined
          // );
        });

        console.log("11", skiptokens);
