...
const MainEmailPage = ({ email }) => {
  ...
  const EmailData = async () => {
    try {
      ...
      if (
        ...
        setPageToken(
          userData.data.paginationData.map((item) =>
            item
              .split("=")
              [userData.data.paginationData[0].split("=").length - 1].replace(
                /%/g,
                ""
              )
          )
        ...
        setTotalData(userData.data.value.length);
        setIsLoading(false);
      }
    } catch (err) {
      console.log(err);
  ...
  console.log(totalData, recordsPerPage);
