...
const PreviewEmail = ({ id }) => {
  ...
  const openPDFInNewTab = (base64PDFData) => {
    const blob = new Blob([base64PDFData], { type: "application/pdf" });
            ...
            <div className="email-content">
              ...
              <div className="attachments">
                {(emailData.attachment[0] || []).map((item, index) => (
                  <>
                    {console.log(item.ext_type)}
                    {item.ext_type === "pdf" ? (
                      <Link
                        onClick={(e) => {
                          openPDFInNewTab(
                            `data:application/pdf;base64,${item.file_data}`
                          );
                        }}
                      >
                        <img
                          key={index}
                          filePath={`https://s.yimg.com/fz/api/res/1.2/O0oW0TRMJLWcDOyAgUZScg--~C/YXBwaWQ9c3JjaGRkO2ZpPWZpdDtoPTEzMDtxPTgwO3c9MTA1/https://s.yimg.com/zb/imgv1/9a8b4cb8-aba2-3b23-b911-7b50ca0139d2/t_500x300`}
                          alt="pdf"
                        />
                      </Link>
                    ) : (
