  ...
  const EmailData = async () => {
    try {
      ...
      if (
        ...
        // Extracting the skiptoken value from the URL
        // const skiptoken = userData.data.paginationData
        //   .map((item) => item["$skiptoken"])
        //   .find((skiptoken) => skiptoken && skiptoken.includes("$skiptoken="))
        //   .split("=")[1];

        // // Removing '%' character from skiptoken
        // const cleanedSkiptoken = skiptoken.replace(/%/g, "");

        console.log(
          userData.data.paginationData.map((item) => item["$24skiptoken"])
          // .find((skiptoken) => skiptoken && skiptoken.includes("$skiptoken="))
        );
