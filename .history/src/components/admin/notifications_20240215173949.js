...
function Notifications({ userId, setDocId, setNotificationDoc }) {
  ...
  const Notiication = async () => {
    try {
      let Response = await getAllMentionNotification(userId); //getAllAdminNotification();
