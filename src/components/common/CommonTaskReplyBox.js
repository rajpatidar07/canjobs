...
import { SendReplyCommit, GetReplyCommit, DeleteReplyCommentsAndAssign, getallAdminData } from '../../api/api';
...
export default function CommonTaskReplyBox(props) {
        ...
        if (props.taskData.id) {
            try {
                let res = await GetReplyCommit("",
                    props.taskData.id /*, adminid, annotationStatus*/
                );
                if (res.data.status === (1 || "1")) {
                    setCommentsReplyList(res.data.data);
                }
                if (res.data.status === (0 || "0") || res.data.message === "data not found") {
    ...
    const OnDeleteReplyComment = async (id) => {
        try {
            let res = await DeleteReplyCommentsAndAssign(id, "", "", admin_id, AdminType);
            ...
            >
                ...
                >
                    <div className="comment-input-container m-0">
                        <label className="input_label m-0">Add Task Reply:</label>
                        <textarea
                            type="text"
                            value={replyComment || ""}
