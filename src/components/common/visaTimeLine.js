...
export default function VisaTimeLine({ visa, substage, type }) {
  ...
  return (
    <div className={type ? "p-1 px-7 col-md-12 d-flex border-right border-bottom" : ""}>
      <div className="arrow-wrapper custome_arrow_wrapper w-100 d-flex flex-wrap mb-0">
        {type && <div className="job_name text-dark">
          <span className={`m-2 font-size-2 d-block mb-1 ${type === "pnp" || type === "pgwp" ? "text-uppercase" : "text-capitalize"}`}>{type}</span>
        </ div >}
